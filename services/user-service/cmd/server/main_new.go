...
func main() {
	...
	if cfg.Registry.Type == "consul" && cfg.Registry.Address != "" {
		serviceID, err = registerService(cfg.Registry, cfg.Server.HTTPPort)
		if err != nil {
	...
	// 初始化服务
	userService := service.NewUserService(userRepo, roleRepo, cfg.Auth.JWTSecret, time.Duration(cfg.Auth.TokenExpiry)*time.Second)
...
func deregisterService(address, serviceID string) {
	...
	log.Printf("Service deregistered (ID: %s)", serviceID)
}
